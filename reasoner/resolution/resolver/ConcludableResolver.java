...
public class ConcludableResolver extends Resolver<ConcludableResolver> {
    ...
    protected RequestState requestStateCreate(Request fromUpstream, int iteration) {
        ...
        if (completableStatesTracker.fullyExplored(fromUpstream.partialAnswer().conceptMap())) {
            ...
            ExplorationRequestState requestState;
            if (completableStatesTracker.traversalExplored(fromUpstream.partialAnswer().conceptMap())) {
                FunctionalIterator<Partial<?>> completedUpstreamAnswers = completableStatesTracker.traversalIterator(fromUpstream.partialAnswer().conceptMap())
                        .map(c -> fromUpstream.partialAnswer().asMapped().aggregateToUpstream(c));
                requestState = new ExplorationRequestState(completedUpstreamAnswers, iteration, false);
            } else {
                ...
                requestState = new ExplorationRequestState(upstreamAnswers, iteration, singleAnswerRequired);
            }
            if (!recursionState.hasReceived(fromUpstream.partialAnswer().conceptMap())) {
